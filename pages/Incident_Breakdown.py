...
if not Manager:
    st.info("Please select  S Manager to get started.")
    st.stop()
...
with incPer:
    st.subheader("Percentage per SM Selection")
...
fig=px.bar(df_pivot,x=df_pivot.index,y=df_pivot.columns,title="Incidents Count Per Customer",labels={'value':'Count','index':'Company',},barmode='stack')
...
fig3=px.bar(df_pivot,x=df_pivot.index,y=df_pivot.columns,title="Priority  Count Per Customer",labels={'value':'Count','index':'Company',},barmode='stack')
